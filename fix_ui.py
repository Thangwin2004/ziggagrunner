...
# 1. Update createWideButtonTexture
def replacer_wide(m):
    ...
    return this.createTextureFromCanvas(
      (ctx, w, h) => {
        ...
        // 1. Solid Shadow (Offset down by 15%)
    ...
    const texture = this.createTextureFromCanvas(
      (ctx, w, h_canvas) => {
        ...
        if (ICONS[iconNameStr]) {
          const p = new Path2D(ICONS[iconNameStr]);
          ctx.save();
...
content = re.sub(r'  createIconButton\(iconName, onClick, scale = 60\) \{.*?(?=\n  createToggleSprite)', replacer_icon, content, flags=re.DOTALL)
