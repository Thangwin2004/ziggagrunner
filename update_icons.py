...
if "getIconBase64(" not in content:
    content = content.replace('  createIconButton(iconName, onClick, scale = 60, theme = "blue") {', get_icon_fn + '\n  createIconButton(iconName, onClick, scale = 60, theme = "blue") {')

# Replace HTML DOM image urls
content = content.replace('"url(/assest/iconbtn/Home_btn.png)"', '`url(${this.getIconBase64("home")})`')
content = content.replace('"url(/assest/iconbtn/replay_btn.png)"', '`url(${this.getIconBase64("replay")})`')
content = content.replace('"url(/assest/iconbtn/continue_btn.png)"', '`url(${this.getIconBase64("play")})`')
content = content.replace('"url(/assest/iconbtn/x2_btn.png)"', '`url(${this.getIconBase64("x2")})`')

# Remove the CSS filter
content = re.sub(r'\s*filter:\s*sepia\(1\).*?;', '', content)

with open('src/ui/UIManager.js', 'w', encoding='utf-8') as f:
    f.write(content)

print('Updated UI to use Base64 dynamic Canvas buttons!')
