...
export class UIManager {
  ...
  createIconButton(iconName, onClick, scale = 60, theme = 'purple') {
    const w = scale * 1.2;
    const h = scale * 1.2;
    const texH = h + h * 0.15 + 4;

    const texture = this.createTextureFromCanvas((ctx, w, h_canvas) => {
        let colorTop, colorBot, colorShadow;
        if (theme === 'green') {
            colorTop = '#66BB6A'; colorBot = '#43A047'; colorShadow = 0x2E7D32;
        ...
        ctx.fillStyle = shadowHex;
        ctx.beginPath();
        ctx.arc(radius, radius + radius * 0.3, radius, 0, Math.PI * 2);
        ctx.fill();
